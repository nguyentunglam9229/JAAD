...
def build_targets(pred_boxes, target, anchors, num_anchors, num_classes, nH, nW, noobject_scale, object_scale, sil_thresh, seen):
    ...
    for b in xrange(nB):
        ...
        for t in xrange(50):
            if target[b][t*5+1] == 0: # if there is no object
                break
            nGT = nGT + 1
            best_iou = 0.0
            best_n = -1
...
class RegionLoss(nn.Module):
    ...
    def __init__(self, num_classes=0, anchors=[], batch=16, num_anchors=1):
        ...
        self.batch = batch
        self.anchors = anchors # anchors' params
    ...
    def forward(self, output, target):
        # output : B*A*(4+1+num_classes)*H*W. size of output
        # B: number of batches
        # A: number of anchors
        ...
        # print('target -1', tcls.size())
        # print('flatten target', tcls.view(-1).size())
        # print('cls_mask', cls_mask.view(-1).size())
        ...
        FL = FocalLoss(class_num=2, gamma=2, size_average=False).cuda()
        
        ##############################
        # print('target 1', tcls)


        loss_cls = self.class_scale * FL(cls, tcls) # class, true class
