...
def load_data_detection(base_path, imgpath, train, train_dur, shape, dataset_use='ucf101-24', jitter=0.2, hue=0.1, saturation=1.5, exposure=1.5):
    ...
    im_ind = int(im_split[num_parts-1][0:5]) # get the image_index (not video index)
    # labpath = os.path.join(base_path, 'annotations_yowo', im_split[0], im_split[1] ,'{:05d}.txt'.format(im_ind))
    labpath = os.path.join(base_path, 'annotations_yowo', im_split[0],'{:05d}.txt'.format(im_ind))
    ...
    if dataset_use == 'ucf101-24':
        max_num = len(os.listdir(img_folder)) # count how many frames in that video
    ...
    for i in reversed(range(train_dur)):
        ...
        if dataset_use == 'ucf101-24':
            ...
            path_tmp = os.path.join(base_path, 'rgb-images', im_split[0], '{:05d}.png'.format(i_temp))
        else:
